...
void drawGui()
{
    ...
    if (ImGui::CollapsingHeader("OBJ Loading/Saving"))
    {
        ...
        if (ImGui::Button("Load OBJ"))
        {
            try
            {
                currentPolyhedron = loadOBJ(objFilePathBuffer);
                currentTransformation.identity();
        ...
        {
            Polyhedron transformedPolyhedron = currentPolyhedron;
            transformedPolyhedron.transform(currentTransformation);
    ...
    if (ImGui::CollapsingHeader("Surface Generation"))
    {
        ...
        {
            currentPolyhedron = generateSurface(surfaceFuncType, surfaceXMin, surfaceXMax, surfaceYMin, surfaceYMax, surfaceXSteps, surfaceYSteps);
            currentTransformation.identity();
        }
        ...
        if (ImGui::Button("Load Surface"))
        {
            try
            {
                currentPolyhedron = loadOBJ(surfaceFilePathBuffer);
                currentTransformation.identity();
