...
Polyhedron createFigureOfRevolution(const std::vector<Point3D> &generatingPoints, char axis, int steps)
{
    ...
    int numGeneratingPoints = generatingPoints.size();
    for (int i = 0; i < steps; ++i)
    {
        for (int j = 0; j < numGeneratingPoints - 1; ++j)
        {
            int p1 = i * numGeneratingPoints + j;
            int p2 = i * numGeneratingPoints + j + 1;
            int p3 = ((i + 1) % steps) * numGeneratingPoints + j + 1;
            int p4 = ((i + 1) % steps) * numGeneratingPoints + j;

            faces.push_back({p1, p2, p4});
            faces.push_back({p2, p3, p4});
        }
    }
