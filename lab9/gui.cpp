...
static char texturePathBuffer[256] = "test.jpg";
...
void drawGui()
{
    ...
    if (ImGui::CollapsingHeader("Object Management"))
    {
        ...
        else
        {
            ...
            if (currentIndex >= 0 && currentIndex < sceneObjects.size())
            {
                float c[3] = {(float)sceneObjects[currentIndex].color.x, (float)sceneObjects[currentIndex].color.y, (float)sceneObjects[currentIndex].color.z};
                if (ImGui::ColorEdit3("Object Color", c))
                {
                    sceneObjects[currentIndex].color.x = c[0];
                    sceneObjects[currentIndex].color.y = c[1];
                    sceneObjects[currentIndex].color.z = c[2];
                }
            }
        }
    }
    ...
    if (ImGui::CollapsingHeader("Rendering"))
    {
        ...
        ImGui::BeginDisabled(!enableLighting);
        const char *shadingModes[] = {"Gouraud", "Toon"};
        int currentShading = static_cast<int>(currentShadingMode);
        if (ImGui::Combo("Shading Mode", &currentShading, shadingModes, IM_ARRAYSIZE(shadingModes)))
        {
            currentShadingMode = static_cast<ShadingMode>(currentShading);
        }

        static bool attachLightToCamera = false;
        ImGui::Checkbox("Attach to camera", &attachLightToCamera);

        if (attachLightToCamera)
        {
            sceneLight.position = camera.position;
        }

        ImGui::BeginDisabled(attachLightToCamera);
        float position[3] = {(float)sceneLight.position.x, (float)sceneLight.position.y, (float)sceneLight.position.z};
        if (ImGui::DragFloat3("Light Position", position, 0.1f))
        {
            sceneLight.position.x = position[0];
            sceneLight.position.y = position[1];
            sceneLight.position.z = position[2];
        }
        ImGui::EndDisabled();

        float color[3] = {(float)sceneLight.color.x, (float)sceneLight.color.y, (float)sceneLight.color.z};
        if (ImGui::ColorEdit3("Light Color", color))
        {
            sceneLight.color.x = color[0];
            sceneLight.color.y = color[1];
            sceneLight.color.z = color[2];
        }
        ImGui::EndDisabled();
    }
    ...
    {
        ImGui::InputText("Texture Path", texturePathBuffer, IM_ARRAYSIZE(texturePathBuffer));
        ImGui::SameLine();
        if (ImGui::Button("Load Texture"))
        {
            if (texture.loadFromFile(texturePathBuffer))
            {
                std::cout << "Texture loaded successfully from " << texturePathBuffer << std::endl;
                textureLoaded = true;
                if (currentIndex >= 0 && currentIndex < sceneObjects.size())
                {
                    sceneObjects[currentIndex].texture = &texture;
                    sceneObjects[currentIndex].hasTexture = true;
                }
            }
            else
            {
                std::cerr << "Failed to load texture from " << texturePathBuffer << std::endl;
                textureLoaded = false;
                if (currentIndex >= 0 && currentIndex < sceneObjects.size())
                {
                    sceneObjects[currentIndex].texture = nullptr;
                    sceneObjects[currentIndex].hasTexture = false;
                }
            }
        }
        if (textureLoaded)
        {
            ImGui::Checkbox("Enable Texturing", &enableTexturing);
        }
    }
